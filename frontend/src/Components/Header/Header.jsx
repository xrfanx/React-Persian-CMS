...
export default function Header() {
  return (
    <div className="sticky top-0 z-10 flex flex-col md:flex-row justify-between items-center w-full p-4 bg-[var(--purple)] text-[var(--white)] h-auto md:h-20 gap-4 md:gap-0 rounded-tr-[2rem] rounded-bl-[2rem]">
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 md:gap-4 w-full md:w-auto">
        <div className="flex items-center gap-2 bg-[var(--white)] rounded-tr-[0.8rem] rounded-bl-[0.8rem] p-2 w-full max-w-xs md:w-80">
          <input
            className="w-full border-0 outline-none bg-transparent text-[var(--purpleHard)]
             text-base placeholder:text-[var(--purple)]"
            type="text"
            placeholder="جستجو ..."
          />
          <button className="flex justify-center items-center shrink-0 text-center bg-[var(--purple)]
            top-0 text-[var(--white)] rounded-lg outline-none border-0 cursor-pointer w-14 h-7 
            transition-all duration-300 ease-out hover:bg-[var(--purpleHard)] hover:border hover:border-[var(--purpleHard)]
